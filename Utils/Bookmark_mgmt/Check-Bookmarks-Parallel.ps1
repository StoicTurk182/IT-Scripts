...
# ============================================================================
# PS7 CHECK
# #Requires is ignored under iex so we enforce it manually
# ============================================================================
...
# ============================================================================
# INTERACTIVE MODE
# Prompts are skipped when parameters are passed directly (file mode)
# ============================================================================
...
if (-not $BookmarkPath) {
    ...
    Write-Host "[1/3] Browser bookmark file"
...
if ($ThrottleLimit -lt 1) {
    ...
    Write-Host "      Higher = faster but more likely to trigger rate limiting"
...
if (-not $ReportPath) {
    $ReportPath = "$env:USERPROFILE\Desktop\bookmark-audit-$(Get-Date -Format 'yyyyMMdd-HHmm').csv"
}
...
if ($auth.Count -gt 0) {
    Write-Host "`n--- Auth-Gated (portal/session URLs - likely alive) ---" -ForegroundColor DarkYellow
...
Write-Host "Open in Excel, fill in FolderOverride and DeleteFlag columns, then run Organise-Bookmarks.ps1 with -AuditCsv pointing at this file.`n"
